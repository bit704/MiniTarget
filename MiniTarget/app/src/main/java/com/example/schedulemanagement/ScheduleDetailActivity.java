...
import com.example.schedulemanagement.bean.ScheduleArrange;
import com.example.schedulemanagement.database.DbHelper;
import com.example.schedulemanagement.database.ScheduleArrangeHelper;
...
public class ScheduleDetailActivity extends AppCompatActivity implements
    ...
    private void initAlarmSpinner() {
        ArrayAdapter<String> sa_adapter = new ArrayAdapter<>(this,
                R.layout.item_select, alarmArray);
        ArrayAdapter<String> type_adapter = new ArrayAdapter<>(this,
                R.layout.item_select, target_typeArray);
        schedule_alarm = findViewById(R.id.schedule_alarm);
        Spinner target_type = findViewById(R.id.type);
        schedule_alarm.setPrompt("请选择提醒间隔");
        target_type.setPrompt("请选择目标类别");
        schedule_alarm.setAdapter(sa_adapter);
        target_type.setAdapter(type_adapter);
        schedule_alarm.setSelection(0);
        target_type.setSelection(0);
        schedule_alarm.setOnItemSelectedListener(new AlarmSelectedListener());
        target_type.setOnItemSelectedListener(new AlarmSelectedListener());
    }

    private int alarmType = 0;
    private String[] alarmArray = {"不提醒", "提前5分钟", "提前10分钟",
            "提前15分钟", "提前半小时", "提前1小时", "当前时间后10秒"};
    private String[] target_typeArray = {"学习","娱乐","考试"};
    ...
    private void enableEdit(boolean enabled) {
        ...
        if (enabled) {
            schedule_time.setBackgroundResource(R.drawable.edit_text_selector);
            schedule_title.setBackgroundResource(R.drawable.edit_text_selector);
            schedule_content.setBackgroundResource(R.drawable.edit_text_selector);
        } else {
